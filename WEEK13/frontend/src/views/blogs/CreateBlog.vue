<template>
  <div class="container is-widescreen">
    ...
    <section class="px-6">
      ...
      <p class="help is-danger" v-if="!$v.images.images">file image is more 1 MB</p>
      ...
      <p class="help is-danger" v-if="!$v.titleBlog.maxLength">title must be at least 25 letters</p>
      <p class="help is-danger" v-else-if="!$v.titleBlog.titleBlog">ตัวอักษรเท่านั้น</p>
    </template>
      </div>
      ...
      <p class="help is-danger" v-if="!$v.contentBlog.minLength">title must be at least 50 letters</p>
    </template>
      </div>
      ...
      <div class="field">
        ...
        <div class="control">
          <input :class="{'is-danger' : $v.reference.$error}" class="input" type="url" v-model="$v.reference.$model" placeholder="e.g. https://www.google.com">
        </div>
        <template v-if="$v.reference.$error">
      <p class="help is-danger" v-if="!$v.reference.url">no url</p>
    </template>
      </div>
      ...
      <div class="columns">
        <div class="column">
          <div class="field">
            ...
            <div class="control">
              <input class="input" :class="{'is-danger' : $v.start_date.$error}" type="date" v-model="$v.start_date.$model">
            </div>
            <template v-if="$v.start_date.$error">
              <p class="help is-danger" v-if="!$v.start_date.fromDate">Wrong startDate</p>
            </template>
          </div>
        </div>
        <div class="column">
          <div class="field">
            ...
            <div class="control">
              <input :class="{'is-danger' : $v.end_date.$error}" class="input" type="date" v-model="$v.end_date.$model">
            </div>
            <template v-if="$v.end_date.$error">
              <p class="help is-danger" v-if="!$v.end_date.toDate">Wrong endDate</p>
            </template>
          </div>
        </div>
      </div>
...
function titleWrong (value) {
 ...
 function toDateWrong(value){
  if(value < this.start_date || (this.start_date == '' && this.end_date != '')){
    return false
  }
  else{
    return true
